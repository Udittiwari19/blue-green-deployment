...
THREADS=4
CONNECTIONS=50
DURATION=180          # total test duration in seconds (2× longer so switchover
                      # is a small fraction of the measurement window)
RATE=50               # req/s — set below system saturation point (~75 req/s).
                      # Each request fans out to 4 backends, so 50 req/s here
                      # = 200 backend calls/s. At 200 req/s the queue grows
                      # unboundedly (2.7× over capacity) making p99 meaningless.
